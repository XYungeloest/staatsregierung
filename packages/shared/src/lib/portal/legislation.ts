...
export interface LegislativeProcedure {
  ...
  relatedNorms: string[];
  /** Common editorial stand derived from packages/shared/src/config/editorial.json. */
  confirmedAsOf: string;
  group?: string;
  dateNote?: string;
}

const CONTENT_ROOT = join(resolveRepositoryRoot(), 'content', 'gesetzgebung');
...
function parseProcedure(value: unknown, path: string): LegislativeProcedure {
  ...
  if ('confirmedAsOf' in entry) {
    fail(`${path}.confirmedAsOf`, 'wird zentral aus packages/shared/src/config/editorial.json abgeleitet und darf nicht im Vorgang wiederholt werden');
  }
