 ...
 *
 * Aufruf:
 *   node scripts/verify-recht-d1.mjs [--local [--persist-to .cache/wrangler-local]] [slug ...]
 *
...
const requestedSlugs = args.filter((value, index) => !value.startsWith('--') && (persistIndex < 0 || index !== persistIndex + 1));

function query(sql) {
  const target = local ? ['--local', '--persist-to', persistTo] : ['--remote'];
  const out = execFileSync('npx', ['wrangler', 'd1', 'execute', 'ostrecht-recht', ...target, '--json', '--command', sql], {
...
const counts = query(`SELECT
  ...
  (SELECT value FROM law_runtime_meta WHERE key='norm_count') AS norm_count_meta`)[0];
...
const publications = readdirSync(join(ROOT, 'content', 'verkuendungen')).filter((file) => file.endsWith('.json')).length;
console.log('Git:', JSON.stringify({ norms: slugs.length, versions: gitVersions, r2_sources: gitR2Sources, publications }));
...
// Stichproben: übergebene Slugs oder deterministische Auswahl über den Bestand
const sample = requestedSlugs.length > 0
  ? requestedSlugs
  : slugs.filter((_, index) => index % Math.max(1, Math.floor(slugs.length / 12)) === 0).slice(0, 12);
