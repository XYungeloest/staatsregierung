...
export interface EditorialFieldDefinition {
  ...
  referenceTarget?: 'person' | 'ministry' | 'government' | 'topic' | 'norm' | 'knowledge-project';
  validation?: {
    maxLength?: number;
    minItems?: number;
    maxItems?: number;
...
export const editorialRegistry: Record<EditorialContentTypeId, EditorialContentTypeDefinition> = {
  ...
  topic: {
    ...
    fields: [slugField, titleField, { name: 'status', label: 'Status', type: 'enum', required: true, enumValues: [{ value: 'geplant', label: 'Geplant' }, { value: 'entwurf', label: 'Entwurf' }, { value: 'im-gesetzgebungsverfahren', label: 'Im Gesetzgebungsverfahren' }, { value: 'beschlossen', label: 'Beschlossen' }, { value: 'in-umsetzung', label: 'In Umsetzung' }, { value: 'abgeschlossen', label: 'Abgeschlossen' }], serialization: 'string' }, { name: 'cluster', label: 'Themenbereich', type: 'enum', required: true, enumValues: [{ value: 'staat-demokratie', label: 'Staat, Demokratie und Recht' }, { value: 'bildung-gesellschaft', label: 'Bildung, Gesundheit und Gesellschaft' }, { value: 'wirtschaft-arbeit', label: 'Wirtschaft und Arbeit' }, { value: 'infrastruktur-wohnen', label: 'Infrastruktur, Regionen und Wohnen' }, { value: 'umwelt-versorgung', label: 'Umwelt, Energie und Versorgung' }, { value: 'nachbarschaft-europa', label: 'Nachbarschaft und Europa' }], serialization: 'string' }, { name: 'priority', label: 'Redaktionelle Priorität', type: 'number', required: true, description: '0 bis 100; steuert nur die Reihenfolge, nicht den fachlichen Status.' }, { name: 'featured', label: 'Dauerhafter Schwerpunkt', type: 'boolean', serialization: 'boolean' }, { name: 'highlightFrom', label: 'Aktuell hervorheben ab', type: 'date', serialization: 'string' }, { name: 'highlightUntil', label: 'Aktuell hervorheben bis', type: 'date', serialization: 'string' }, { name: 'updatedAt', label: 'Fachlicher Stand', type: 'date', required: true, serialization: 'string' }, { name: 'federfuehrendesRessort', label: 'Federführendes Ressort', type: 'ministry-reference', required: true, referenceTarget: 'ministry', serialization: 'string' }, { name: 'mitzeichnungsressorts', label: 'Mitzeichnende Ressorts', type: 'reference-list', referenceTarget: 'ministry', serialization: 'array' }, { name: 'relatedTopicSlugs', label: 'Verwandte Themen', type: 'reference-list', referenceTarget: 'topic', serialization: 'array' }, { name: 'teaser', label: 'Kurzbeschreibung', type: 'textarea', required: true, serialization: 'string' }, { name: 'beschlossen', label: 'Beschlossen', type: 'sortable-list', required: true, serialization: 'array' }, { name: 'umgesetzt', label: 'Umgesetzt', type: 'sortable-list', required: true, serialization: 'array' }, { name: 'naechsteSchritte', label: 'Nächste Schritte', type: 'sortable-list', required: true, serialization: 'array' }, { name: 'keyDates', label: 'Wichtige Termine', type: 'object-list', serialization: 'object-array' }, { name: 'modules', label: 'Inhaltsspezifische Module', type: 'object-list', serialization: 'object-array' }, { name: 'knowledgeProjectRefs', label: 'Wissenshub-Projekte', type: 'reference-list', referenceTarget: 'knowledge-project', description: 'Auswahl und Coverage-Register werden gemeinsam in einem Commit aktualisiert.', serialization: 'array' }, { name: 'rechtsgrundlagen', label: 'Rechtsgrundlagen', type: 'object-list', required: true, serialization: 'object-array' }, { name: 'faq', label: 'Fragen und Antworten', type: 'object-list', required: true, serialization: 'object-array' }],
